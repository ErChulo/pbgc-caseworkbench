...
The app includes task-level evidence guidance inside **Case Workflow** for this purpose. Each active task states what fact family is needed, which IVS document classes to search, which scraper or JSON contract to use, what manual fallback is acceptable, what the app can validate, and which downstream artifact is unlocked. Normal use should follow the single primary action shown for the current workflow step. The full evidence reference can be downloaded as `case-evidence-guide.json`.
